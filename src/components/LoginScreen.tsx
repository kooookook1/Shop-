  ...
  return (
    <div className="relative min-h-screen w-full overflow-hidden flex flex-col justify-between px-6 py-12 bg-gradient-to-b from-[#0a2562] to-[#051b4d]">
      ...
      <div className="flex-1 flex flex-col justify-center items-center max-w-sm mx-auto w-full z-10 py-6">
        ...
        <form onSubmit={handleSubmit} className="w-full space-y-4">
          {isRegistering && (
            ...
            >
              ...
              <input 
                ...
                className="w-full bg-white/5 border border-white/10 rounded-2xl py-4 pr-12 pl-4 text-right text-sm placeholder-white/30 text-white focus:outline-none focus:ring-1 focus:ring-brand-gold hover:bg-white/10 transition-colors"
              />
            </motion.div>
          )}
          ...
          <motion.button 
            ...
            whileTap={{ scale: 0.98 }}
            type="submit"
            className="w-full bg-gradient-to-b from-amber-200 via-amber-400 to-amber-600 text-[#051b4d] font-bold py-4 rounded-2xl shadow-lg shadow-amber-500/10 mt-6 transition-all border border-amber-300/30 text-center flex items-center justify-center gap-2"
          >
        ...
        <div className="mt-12 text-center w-full">
          ...
          <div className="flex justify-center gap-5">
            <button 
              type="button"
              onClick={() => onLogin('أحمد محمد (Google)', 'google_user@gmail.com')}
              className="w-12 h-12 rounded-full bg-white flex items-center justify-center hover:bg-slate-100 transition-colors shadow-md border border-white/10"
            >
            ...
            <button 
              type="button"
              onClick={() => onLogin('أحمد محمد (Apple)', 'apple_user@icloud.com')}
              className="w-12 h-12 rounded-full bg-black flex items-center justify-center hover:bg-slate-900 transition-colors shadow-md border border-white/10"
            >
      ...
      <div className="w-full flex justify-between text-xs text-white/50 max-w-sm mx-auto z-10 select-none">
        {isRegistering ? (
          <button type="button" onClick={() => setIsRegistering(false)} className="hover:text-amber-300 transition-colors">لديك حساب؟ تسجيل الدخول</button>
        ) : (
          <button type="button" onClick={() => setIsRegistering(true)} className="hover:text-amber-300 transition-colors">إنشاء حساب جديد</button>
        )}
        <button type="button" className="hover:text-amber-300 transition-colors">نسيت كلمة المرور؟</button>
      </div>
    </div>
  );
}
